...
func GetPokemon(name string, client *http.Client, cache *pokecache.Cache) (Pokemon, error) {
	...
	if ok {
		if err := json.Unmarshal(data, &pokemonData); err != nil {
			return pokemonData, fmt.Errorf("error getting pokemon data cache")
		}
		fmt.Println("pokemon data retrieved from cache")
	...
	if err != nil {
		return pokemonData, fmt.Errorf("error creating api request: %s", err)
	}

	res, err := client.Do(req)
	if err !=nil {
		return pokemonData, fmt.Errorf("error getting api response: %s", err)
	}

	defer res.Body.Close()

	if res.StatusCode != 200 {
		return pokemonData, fmt.Errorf("not found")
	}

	newData, err := io.ReadAll(res.Body)
	if err != nil {
		return pokemonData, fmt.Errorf("error reading response: %s", err)
	}

	if err := json.Unmarshal(newData, &pokemonData); err != nil {
		return pokemonData, fmt.Errorf("error decoding api response %s", err)
	}
